...
class SetDeployedVersionAPIView(View):
    ...
    def post(self, request):
        try:
            ...
            name = data.get("name")
            version = data.get("version")
            deployment_point = data.get("deployment_point")
            
            if not name or not version or not deployment_point:
                return JsonResponse({"success": False, "error": "name, version, and deployment_point are required"}, status=400)
                
            self.service.set_deployed_version(name, version, deployment_point)
