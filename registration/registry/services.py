...
class MLflowService:
    def __init__(self):
        # Resolve .env file using absolute path relative to this file
        # This file is at registration/registry/services.py
        # .env is at ../../.env
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        ...
        for rm in rms:
            ...
            # Match existing API expected structure which is a list of models with versions
            results.append({
                "name": rm.name,
    ...
    def delete_model_version(self, name: str, version: int) -> bool:
        try:
            # Check if this run is uniquely used by this version so we can delete the run too
            mv = self.client.get_model_version(name, str(version))
            run_id = mv.run_id
            ...
            # If a run ID is present, we could delete it if no other MV uses it
            # But skipping for safety right now, MLflow doesn't strictly require run deletion.
            return True
