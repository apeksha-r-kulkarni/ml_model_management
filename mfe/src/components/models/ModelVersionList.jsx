...
export default function ModelVersionList({ versions, currentlyDeployedVersion, onEdit, onDelete, onSetDeployed }) {
  return (
      ...
      <table className="w-full text-left border-collapse">
        ...
        <tbody className="divide-y divide-gray-100 text-sm">
          {versions.map((v, idx) => {
            ...
            const isDeployed = currentlyDeployedVersion === v.version;
            ...
            return (
              <tr key={v.id || idx} className="hover:bg-blue-50/30 transition-colors">
                ...
                <td className="p-3.5 whitespace-nowrap space-x-1">
                  ...
                  {v.is_deployable && (
                    <span className="px-2 py-0.5 rounded text-xs font-bold bg-gray-100 text-gray-500 border border-gray-200" title="Deployment tracking is not currently supported in this backend">
                      DEPLOYABLE
                    </span>
                  )}
                </td>
